...
Optional Arguments:
    --received_log: File path for outputting filtered (received) messages. (Default: received_messages.log)
    --debug_log: File path for outputting all other (debug) logs. (Default: debug_messages.log)
...
received_handler.setLevel(logging.INFO)  # We log the filtered messages as INFO
...
def onReceive(packet, interface):
    ...
    Logs JSON dump of message info only if it matches the filtered sender and port.
    ...
    # If the message is from the filtered sender and intended as text message:
    if raw_sender_id == SENDER_NODE_ID and portnum == "TEXT_MESSAGE_APP":
        ...
        # Flush file handlers immediately.
        for handler in logger.handlers:
            handler.flush()
        print("Logged filtered message:", message_json)
    else:
        # Any other message gets logged as debug.
...
# Subscribe with the expected callback signature.
...
def check_connection(interface):
    """
    Sends a connection check message.
