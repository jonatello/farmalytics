...
class MeshtasticSender:
    """
    Encapsulates file reading, chunking, and sending via Meshtastic.
    ...
    def generate_async_header(self, index: int) -> str:
        ...
        if '#' in template:
            # Use regex to find a run of '#' characters.
    ...
    async def send_chunks_async(self, chunks: list) -> tuple:
        """
        Asynchronously send all chunks concurrently.
        ...
        for i, chunk in enumerate(chunks, start=1):
            ...
            task = asyncio.create_task(self.send_chunk_async_with_header(chunk, header))
            tasks.append(task)
